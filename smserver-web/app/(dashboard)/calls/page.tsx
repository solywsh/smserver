...
import { api, CallLogWithDevice, Device } from '@/lib/api';
...
export default function AllCallsPage() {
  ...
  const fetchCalls = async () => {
    ...
    const res = await api.getAllCalls(type, page, pageSize, searchQuery || undefined, deviceId);
    if (res.data) {
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">View call history from all devices</p>
        </div>
      </div>
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              {searchQuery || typeFilter !== '0' ? 'No calls found with this filter' : 'No call logs yet.'}
            </div>
          ) : (
